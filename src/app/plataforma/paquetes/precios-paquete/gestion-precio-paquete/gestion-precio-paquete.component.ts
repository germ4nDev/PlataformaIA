...
export class GestionPrecioPaqueteComponent implements OnInit, OnDestroy {
    ...
    isSubmit: boolean = false; // Controla si se intentó guardar (para mostrar errores)
    isSaving: boolean = false; // Controla el spinner mientras la API responde

    modoEdicion: boolean = false;
    codigoPaquete: string = '';
    nombrePaquete: string = ''; // Para el Breadcrumb
    listaId: string = '';
    detalleId: string = '';
    ...
    constructor(
        ...
        private route: ActivatedRoute,
        private translate: TranslateService,
        private fb: FormBuilder, // 🟢 Inyección del FormBuilder
        private _registrosService: PTLListasPreciosService,
        private _layoutInitializer: LayoutInitializerService,
        private _logActividadesService: PtllogActividadesService,
        private _swalAlertService: SwalAlertService,
        ...
        // Recuperamos los IDs de la pantalla anterior
        this.codigoPaquete = this._localStorageService.getObject<string>('regId') || '';
        this.nombrePaquete = this._localStorageService.getObject<string>('nombrePaquete') || 'Paquete Seleccionado';
        this.listaId = this._localStorageService.getObject<string>('listaId') || '';
        this.detalleId = this._localStorageService.getObject<string>('detalleId') || 'nuevo';
    ...
    // 🟢 Getter conveniente para acceder a los controles en el HTML
    get f() { return this.formularioRegistro.controls; }

    inicializarFormulario() {
        this.formularioRegistro = this.fb.group({
            costoBaseMensual: [null, [Validators.required, Validators.min(0)]],
            valorBaseMensual: [null, [Validators.required, Validators.min(0)]],
            precioSetup: [0, [Validators.min(0)]]
        });
    }
    ...
    ngOnInit() {
        ...
        this.lockScreenSubscription = this._navigationService.lockScreenEvent$.subscribe({
            next: (message: string) => {
                // Guardamos el estado del formulario reactivo
    ...
    cargarDatosEdicion() {
        // En una app real, si no tuviéramos los datos, haríamos un GET aquí.
        // Por ahora, asumiremos que el usuario digitará o traeremos la data del store.
    }
    ...
    btnGestionarRegistroClick() {
        ...
        // 🟢 3. Validación elegante
        if (this.formularioRegistro.invalid) {
            this._swalAlertService.getAlertError(this.translate.instant('PLATAFORMA.FORMULARIOINVALIDO'));
            return;
        }

        this.isSaving = true; // Activamos el spinner del botón
        const formValues = this.formularioRegistro.value;

        const dataGuardar = {
            tipoReferencia: 'PAQUETE',
            codigoReferencia: this.codigoPaquete,
            costoBaseMensual: formValues.costoBaseMensual,
            valorBaseMensual: formValues.valorBaseMensual,
            precioSetup: formValues.precioSetup || 0,
            estadoDetalle: true
        };
    ...
    // 🟢 Refactorización para no repetir código de éxito/error
