...
import { PTLEmpresaSCModel } from '../../../theme/shared/_helpers/models/PTLEmpresaSC.model';
import { PtlEmpresasScService } from '../../../theme/shared/service/ptlempresas-sc.service';
//#endregion IMPORTS
...
export class UsuariosRolesComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        this.suscriptor = this.suscriptores.find(x => x.codigoSuscriptor == this.codigoSuscriptor) || {}
        console.log('datos usuario', this.usuario);
        this.tituloPagina = this.translate.instant('SUSCRIPTORES.USUARIOSROLES.TITLE') + ' ' + this.usuario.nombreUsuario;
        this.subtituloPagina = this.translate.instant('SUSCRIPTORES.USUARIOSROLES.TITLE') + ' ' + this.suscriptor.nombreSuscriptor;
        ...
        this.registrosTransformados$ = this._usuariosRolesService._usuariosRoles$.pipe(
            switchMap((usuariosRoles: PTLUsuarioRoleAPModel[]) => {
                ...
                const codigosSCDelUsuario = this.usuariosSC
                    .filter(x => x.codigoUsuario === this.codigoUsuario)
                ...
                const transformedApps = rolesDelUsuario.map((usuarioRole: any) => {
                    const isActivo = usuarioRole.estadoUsuarioRole;

                    usuarioRole.nomEstado = usuarioRole.estadoUsuarioRole ? 'Activo' : 'Inactivo';

                    const empresaAsignado = this.empresasSC.find(x => x.codigoEmpresaSC === usuarioRole.codigoEmpresaSC);
                    usuarioRole.nomEmpresa = empresaAsignado ? empresaAsignado.nombreEmpresa : 'N/A';
                    ...
                    return {
                        ...
                        '_acciones': [{
                            accion: 'INACTIVAR',
                            letra: isActivo ? 'I' : 'A',
                            color: isActivo ? '#13af2d' : '#13af2d',
                            tooltip: isActivo ? this.translate.instant('USUARIOSROLES.INACTIVAR') : this.translate.instant('USUARIOSROLES.ACTIVAR')
                        }],
    ...
    columnasRegistros: ColumnMetadata[] = [
        {
            name: 'nomEmpresa',
            header: 'USUARIOS.USUARIOSROLES.NOMBREEMPRESA',
            type: 'text'
        },
        {
    ...
    OnNuevoRegistroClick() {
        ...
        this.router.navigate(['/suscriptor/gestion-usuario-roles'])
    }
        ...
        switch (accion) {
            case 'INACTIVAR':
                this._swalAlertService.getAlertQuestionRequest(
                    this.translate.instant('ROLESUSUARIOS.INACTIVAR'),
                    this.translate.instant('ROLESUSUARIOS.INACTIVARTITULO'),
                    this.translate.instant('ROLESUSUARIOS.INACTIVARBTN'),
